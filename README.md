...
**能接手实际任务。** QQ 里主人明确要求写程序、运行代码、生成图片/报告/文件或反复调试时，她会把工作交给后台代码任务：独立目录、真实运行、按输出迭代，完成后主动回传文字和产物。→ [后台代码任务](docs/后台代码任务.md)
...
在 QQ 中先完成主人认证，再直接说“写一个程序”“生成一张图”“整理成报告”或其他明确的产物要求。模型会调用 `code_task` 创建后台任务，在 `data/code_tasks/<任务编号>/` 中使用 `fs_*`、`web_search` 和 `run_python` 完成工作；任务结束后会主动发回说明，并尝试上传产物。

同一会话里可以说“任务状态”“继续做……”或“取消任务”。普通聊天不会创建任务，群友也不能启动任务。第一阶段的 Python 执行属于工具层目录边界，不是操作系统安全沙箱；不会自动注入 API key，缺凭据时会停下来向 bot 管理员说明需要什么。→ [后台代码任务](docs/后台代码任务.md)
...
| [后台代码任务](docs/后台代码任务.md) | QQ 中的后台实现、执行、回传和边界 |
