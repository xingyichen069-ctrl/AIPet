...
本地版本：`0.4.0-beta.1`。更新记录见 [小日和施工日志](CHANGELOG.md)，版本留档与回退方式见 [版本矿坑](docs/版本管理.md)。Windows 便携包请先看 [Windows 使用说明](Windows使用说明.md)。

> 本地增强版：已加入对话接续、约定提醒、安静陪伴、记忆操作、工作状态动作和文本附件。使用方法见 [新增功能说明](新增功能说明.md)。下面保留原版文档，原有界面说明可能与新版不同。

### 对话与右键菜单主题

在「右键桌宠 → 外观 → 主题」或「对话窗 ··· → 外观 → 主题」选择：

- **东方 · 朱色结界**：原版朱红、米白与金色，阴阳玉和御币装饰。
- **雾雨魔理沙 · 星屑魔法**：黄、黑、白，星形徽章与星屑装饰。
- **古明地恋 · 无意识之庭**：黄、绿、蓝，闭眼爱心与蓝色连线装饰。

对话窗、右键主菜单、各层子菜单与托盘菜单同步切换；保留日夜自动配色（07:00–18:00 为日间）。主题选择自动保存，草稿、附件与对话不受切换影响。也可继续调整字号和玻璃质感。详细说明见 [主题使用说明](docs/主题使用说明.md)。
