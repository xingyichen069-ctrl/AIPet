...
先说清楚：**直接把新版 zip 解压覆盖到旧目录，什么都不会丢。** 仓库的 zip 根本不含 `persona/` `memory/` `data/`（都在 `.gitignore` 里），而解压不会删掉压缩包里没有的文件。

需要搬的是另一种换法：**解压到一个新目录，然后把旧的整个丢掉**。那时候人格、记忆、密钥、QQ 绑定、你调过的参数全在新目录里没有。

把【旧版目录】拖到 `迁移私人内容.bat` 上就行。或者：

```bash
python tools/migrate.py D:\旧版\AIPet --dry-run   # 只看清单，不动
python tools/migrate.py D:\旧版\AIPet             # 看清单，问你要不要搬
```

搬的是**代码之外、重建不出来**的东西：人格、记忆、`secrets.json`（API key + QQ 凭据）、`qq.json`（主人绑定）、config / thinking / 外观、心理点日志、约定、知识库、历史备份。

不搬的是**能重建或不该带的**：运行环境（新装跑一次 `准备环境.bat`）、搜索缓存、运行时状态文件 —— `qq_status.json` 搬过去会让桌宠以为 QQ 还连着。

两点安全设计：真开搬之前，目标里被覆盖的文件先存进 `backups/迁移前-<时间>/`；目标里已经有私人内容的话，不加 `--force` 它拒绝动手。

搬完**还有一样要手动改**：开机自启里的快捷方式。脚本会告诉你哪个 `.lnk` 还指着旧目录。

> `secrets.json` 在这里是要搬的，但 `src/backup.py` 故意把它排除在外 —— 那个 zip 可能被拷走、发出去，密钥不该跟着跑。这里不一样，是同一台机器上的目录到目录。
